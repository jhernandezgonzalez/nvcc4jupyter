...
class NVCCPluginV2(Magics):
    ...
    @staticmethod
    def compile(output_dir, file_paths, out, flags=[]):
        res = subprocess.check_output( [compiler, '-I' + output_dir] + flags +
                                       [file_paths, "-o", out, '-Wno-deprecated-gpu-targets'], stderr=subprocess.STDOUT)
        print(res)
    ...
    @argument('-t', '--timeit', type=bool, help='Measure time?')
    @cell_magic
    ...
    @magic_arguments()
    @argument('-f', '--flag', type=str, help='flag for nvcc compiler')
    @argument('-t', '--timeit', type=bool, help='Measure time?')
    @cell_magic
    def cuf(self, line='', cell=None):
        args = parse_argstring(self.cuf, line)
        ...
        flags=args.flag.split("+")

        with tempfile.TemporaryDirectory() as tmp_dir:
            file_path = os.path.join(tmp_dir, str(uuid.uuid4()) + '.cu')
            with open(file_path, "w") as f:
                f.write(cell)

            try:
                self.compile(self.output_dir, file_path, self.out, flags)
                output = self.run(timeit=args.timeit)
            except subprocess.CalledProcessError as e:
                helper.print_out(e.output.decode("utf8"))
                output = None
